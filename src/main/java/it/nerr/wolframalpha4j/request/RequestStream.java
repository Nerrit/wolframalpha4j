...
import it.nerr.wolframalpha4j.http.client.ClientRequest;
...
import it.nerr.wolframalpha4j.util.LogUtil;
...
/**
 * Facilitates the routing of {@link WebRequest} instances to the proper {@link RequestStream} according to
 * the bucket in which the request falls.
 */
public class RequestStream {

    private static final Logger LOGGER = Loggers.getLogger(RequestStream.class);
        ...
        RequestStream(BucketKey id, RouterOptions routerOptions, WebClient httpClient,
            ...
            return Retry.withThrowable(reactor.retry.Retry.onlyIf(ClientException.isRetryContextStatusCode(500, 502, 503,
                    ...
                    .doOnRetry(ctx -> {
                        if (LOGGER.isDebugEnabled()) {
                            LOGGER.debug("Retry {} in bucket {} due to {} for {}",
...
private class RequestSubscriber extends BaseSubscriber<RequestCorrelation<ClientResponse>> {
        ...
        this.responseFunction = response -> {
            ...
            if (LOGGER.isDebugEnabled()) {
                Instant requestTimestamp =
                        Instant.ofEpochMilli(httpResponse.currentContextView().get(WebClient.KEY_REQUEST_TIMESTAMP));
                Duration responseTime = Duration.between(requestTimestamp, Instant.now());
                LogUtil.traceDebug(LOGGER, trace -> format(httpResponse.currentContextView(),
            ...
            if (!resetAfter.isZero()) {
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug(format(httpResponse.currentContextView(), "Delaying next request by {}"), resetAfter);
                }
            ...
            if (global) {
                ...
                action = globalRateLimiter.rateLimitFor(fixedBackoff)
                        .doOnTerminate(() -> LOGGER.debug(format(httpResponse.currentContextView(),
    ...
    protected void hookOnNext(RequestCorrelation<ClientResponse> correlation) {
        WebRequest request = correlation.getRequest();
        ...
        Mono.just(clientRequest)
                .flatMap(req -> Mono.deferContextual(ctx -> {
                    LogUtil.traceDebug(LOGGER, trace -> format(ctx, trace ? req.toString() : req.getDescription()));
                ...
                .subscribe(
                        ...
                        t -> {
                            LOGGER.trace("Error while processing {}: {}", request, t);
                            callback.emitError(t, FAIL_FAST);
                        },
                        () -> callback.emitEmpty(FAIL_FAST));
    }

    private Function<Mono<ClientResponse>, Mono<ClientResponse>> getResponseTransformers(WebRequest discordRequest) {
    ...
    private void next(SignalType signal) {
        ...
        timer
                ...
                .subscribe(l -> {
                    if (LOGGER.isDebugEnabled()) {
                        LOGGER.debug("[B:{}] Ready to consume next request after {}", id.toString(), signal);
                    }
                    request(1);
                }, t -> LOGGER.error("[B:{}] Error while scheduling next request", id.toString(), t));
    }

    @Override
    protected void hookOnComplete() {
        LOGGER.debug("[B:{}] RequestStream completed", id.toString());
    }
}
}
